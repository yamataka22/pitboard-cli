...
```
      --all                             ページングを最後まで辿る
      --archived string                 none（アーカイブ済みを除く）| only（アーカイブ済みだけ）| all（既定 none。--progress-changed-* 指定時は all）
      --assignee string                 メンバー ID または名前 | me | none
  -h, --help                            help for list
      --kind string                     task | issue | all（省略時は両方。ただし --state backlog は task だけ）
      --label strings                   ラベル ID または名前（繰り返し指定で AND）
      --mine                            --assignee me の別名
      --order string                    asc | desc（既定は --sort による）
      --owner string                    メンバー ID または名前 | me
      --page int                        ページ番号
      --per-page int                    1ページの件数（最大 200）
      --point string                    h1 | h4 | d1 .. d5 | none
      --progress string                 進捗カラム ID または名前
      ...
      --project string                  プロジェクト ID または名前 | none
